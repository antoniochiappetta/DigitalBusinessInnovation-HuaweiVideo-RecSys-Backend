...
class PaginatedAPIMixin(object):
    ...
    def to_collection_dict(query, page, per_page, endpoint, **kwargs):
        ...
        return PaginatedAPIMixin.get_collection_dict(resources, endpoint, **kwargs)

    @staticmethod
    def get_collection_dict(pagination_object, endpoint, **kwargs):
        page = pagination_object.page
        per_page = pagination_object.per_page
        data = {
            'items': [item.to_dict() for item in pagination_object.items],
            '_meta': {
                'page': page,
                'per_page': per_page,
            ...
            '_links': {
                ...
                'next': url_for(endpoint, page=page + 1, per_page=per_page,
                                **kwargs) if pagination_object.has_next else None,
                'prev': url_for(endpoint, page=page - 1, per_page=per_page,
                                **kwargs) if pagination_object.has_prev else None
            }
        }
        return data
